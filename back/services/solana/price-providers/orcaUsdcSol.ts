/**
 * Orca devnet USDC/SOL quote provider used by the Compass MCP Guard price
 * layer (`priceQuote.ts`). Extracted from the legacy `tools/orcaSwap.ts`
 * during Wave 3.5 so the guard does not depend on chat-app tools.
 *
 * Keeps the same public shape (`OrcaSwapParams`, `OrcaSwapQuote`,
 * `quoteOrcaUsdcToSol`, devnet mint constants) so internal callers do not
...
import { getConnection } from '../providers/solanaConnection';
