 ...
 * - Invalid JSON, schema mismatch, and timeout preserve the deterministic decision.
 * - LLM output cannot loosen deterministic decisions (decision clamp).
 */
...
export function clampLlmDecision(
	...
	if (allowedStrictness.includes(llmOutput.decision)) {
		// LLM kept or tightened the decision.
		return {
	...
	// LLM tried to loosen the decision - fall back to deterministic.
	return {
		decision: deterministicDecision,
		llmConsulted: true,
		llmOutput,
...
/**
 * Call the LLM judge if enabled and configured. Returns undefined if
 * disabled, unconfigured, timeout, or error.
 */
...
// ---------------------------------------------------------------------------
// Top-level: evaluate with optional LLM metadata
// ---------------------------------------------------------------------------
...
/**
 * Evaluates the LLM judge and returns a clamped decision.
 * If LLM is disabled/unconfigured/errored, returns the deterministic decision unchanged.
 */
export async function evaluateLlmMetadata(
	...
	return clampLlmDecision(
		params.input.deterministicDecision,
		llmOutput,
	);
}