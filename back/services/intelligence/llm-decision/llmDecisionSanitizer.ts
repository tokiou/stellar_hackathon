/**
 * LLM Decision Sanitizer - redacts sensitive data before sending to the LLM judge.
 *
 * Removes/redacts: prompts, raw tx bytes, keys, secrets, and oversized values.
 * Produces an LlmJudgeInput with `sanitized: true`.
 */
...
export type SanitizeLlmInputParams = {
	...
	/** Raw tool arguments or other context to sanitize. */
...
function sanitizeValue(value: unknown, depth: number): unknown {
	...
	// Buffers, Uint8Arrays, and other non-serializable types are redacted.
	return LLM_REDACTED;
}
...
function sanitizeObject(
	...
	for (const [key, value] of Object.entries(obj)) {
		...
		// Redact binary-like values (Uint8Array, Buffer, ArrayBuffer)
		if (
