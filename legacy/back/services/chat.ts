...
import {
	buildUnsignedOrcaSwapTx,
	buildUnsignedOrcaSwapTxWithGuard,
} from "./tools/orcaSwapTx";
import {
	buildSwapGuardConfig,
	type SwapGuardConfig,
	type SwapGuardWarning,
} from "./tools/swapGuard";
import {
...
import {
	buildSwapGuardInstructions,
	getGuardProgramId,
	getPythOracleFeed,
} from "./tools/swapGuardOnChain";
import {
...
import {
	buildTransferAuditEvent,
	buildTransferGatewayApprovalMetadata,
	evaluateTransferGateway,
	verifyTransferGatewayMetadata,
} from "./transferGateway";
import { recordTransferAuditEvent } from "./transferAuditLog";
import {
	TRANSFER_AUDIT_LIFECYCLES,
	type BuildTransferAuditEventInput,
	type TransferAuditEvent,
	type TransferAuditLifecycle,
...
type AgentFunctionCallMessage = {
	...
	execution?: {
		mode:
			| "phantom_sign_and_send"
			| "phantom_execute_then_optional_backend_proof";
...
const ORCA_SWAP_TOOL: ResponsesToolDefinition = {
	...
	parameters: {
		...
		properties: {
			...
			input_amount: {
				type: "number",
				description: "Monto de USDC a convertir",
			},
			slippage_bps: {
				type: "number",
				description: "Slippage en bps (default 100 = 1%)",
			},
		},
...
const GET_USDC_SOL_QUOTE_TOOL: ResponsesToolDefinition = {
	...
	description:
		"Consulta una cotizacion devnet USDC/SOL de solo lectura para contexto del agente.",
	parameters: {
		type: "object",
		properties: {
			input_token: {
...
export const ALL_TOOLS = [
	TRANSFER_TOOL,
	CONDITIONAL_BUY_SOL_TOOL,
	ORCA_SWAP_TOOL,
	GET_WALLET_HOLDINGS_TOOL,
	GET_USDC_SOL_QUOTE_TOOL,
];
...
const ACTION_APPROVAL_ACCOUNT_SPACE =
	8 + 32 + 32 + 32 + 1 + 8 + 8 + 2 + 32 + 8 + 32 + 8 + 1 + 1 + 1;
...
async function getSessionWithExternalFallback(
	sessionId: string,
...
function hasSessionWalletMismatch(
	...
	return Boolean(
		session.userAddress &&
			(!normalizedUserAddress || session.userAddress !== normalizedUserAddress),
	);
}
	...
	return jsonResponse(
		{
			error: {
				...
				message: "A verified app session is required for this action",
			},
		},
		{ status: 401 },
	);
	...
	return jsonResponse(
		{
			error: {
				...
				message: "The request wallet does not match the authenticated wallet",
			},
		},
		{ status: 403 },
	);
...
function hasAuthenticatedRequestMismatch(
	auth: ChatAuthContext,
	userAddress?: string,
): boolean {
	const normalizedUserAddress = userAddress?.trim();
	return Boolean(
		auth.identity &&
			...
			normalizedUserAddress !== auth.identity.walletAddress,
	);
}
...
function authorizeSessionAccess(
	...
	if (auth.identity) {
		return session.userAddress &&
			session.userAddress !== auth.identity.walletAddress
	...
	return hasSessionWalletMismatch(session, requestUserAddress)
		? sessionNotFoundResponse()
		: null;
}
...
function addSessionMessagesFromAgentMessages(
	...
	const historyEntries: SessionHistoryMessageInput[] = messages.map(
		(message) => {
			if (message.type === "function_call") {
				return {
					role: "agent",
...
function buildProposalPayloadFromState(
	pendingProposal: PendingProposal,
): Omit<SessionFunctionMessage, "id" | "timestamp"> {
	const toolName =
		pendingProposal.toolName as AgentFunctionCallMessage["function"]["name"];
	return {
		role: "agent",
		type: "function_call",
		function: {
...
function getRequiredUsdcForConditionalBuy(
	args: ConditionalBuySolParams,
): number {
...
function uiAmountFromBaseUnits(
	baseUnits: string,
	token: "USDC" | "SOL",
): number {
...
async function buildUsdcFundingSwap(
	requiredUsdc: number,
...
function normalizeChatError(err: unknown): { code: string; message: string } {
	...
	if (rawMessage.includes("content_filter")) {
		return {
			code: "content_filter",
			message:
				"El proveedor de IA bloqueó ese pedido. Probá reformularlo con monto, token y destinatario.",
		};
	}
...
export function maskSolanaAddressesForModel(
	content: string,
): MaskedSolanaAddresses {
	const addressByPlaceholder: Record<string, string> = {};
	let index = 0;
	const contentWithMasks = content.replace(
		/[1-9A-HJ-NP-Za-km-z]{32,44}/g,
		(candidate) => {
			const existing = Object.entries(addressByPlaceholder).find(
...
export function parseDirectTransferIntent(
	content: string,
): DirectTransferIntent {
	const normalized = content.trim();
	if (
		!/\b(manda|mand[aá]|envi[aá]|envia|transfer[ií]|transferir|send)\b/i.test(
			normalized,
		)
	) {
...
export function restoreMaskedSolanaAddressesInToolArgs(
	...
	if (!rawArguments || Object.keys(addressByPlaceholder).length === 0)
		return rawArguments;
...
export function evaluateSolTransferFunding(params: {
	...
	const missingLamports = Math.max(
		0,
		requiredLamports - params.balanceLamports,
	);
...
function deriveTransferRecipientKnown(safety: WalletSafetyEvaluation): boolean {
	if (safety.offchainSignals.internalLists.onUserAllowlist) return true;
	if (safety.offchainSignals.solscan?.hasHistory === true) return true;
	if (
		safety.offchainSignals.history?.estimatedAccountAgeBucket === "established"
	)
		return true;
	if (
		safety.offchainSignals.history?.txCountBucket === "medium" ||
		safety.offchainSignals.history?.txCountBucket === "high"
	) {
		return true;
	}
...
function buildTransferWalletSafetyEvidence(
	safety: WalletSafetyEvaluation,
): TransferWalletSafetyEvidence {
	const criticalReason = safety.decisionResult.reasons.some(
		(reason) => reason.severity === "critical",
	);
	const suspiciousRecipient =
		criticalReason ||
		safety.offchainSignals.reputation.severity === "critical" ||
		safety.offchainSignals.abuseReports.some(
			(report) => report.severity === "critical",
		) ||
		safety.offchainSignals.internalLists.onBlocklist ||
		safety.offchainSignals.internalLists.onUserDenylist ||
		safety.offchainSignals.sanctions?.matched === true;

	return {
		status: safety.decisionResult.decision,
		reasonCodes: safety.decisionResult.reasons.map((reason) => reason.code),
		flags: {
			suspicious_recipient: suspiciousRecipient,
		},
	};
}

function buildTransferPolicyErrorPayload(
	evaluation: TransferGatewayEvaluation,
) {
	const failClosedReason =
		evaluation.failClosedReason ?? "policy_unhandled_decision";

	return {
		code: `transfer_policy_${failClosedReason}`,
		message:
			"Compass bloqueó la transferencia antes de crear la propuesta porque la policy requiere más contexto, simulación, actualización o deniega la acción.",
		suggested_action:
			"Revisá los motivos de policy y reintentá con más contexto o un destino permitido.",
		policy_id: evaluation.policyEvaluation.policyId,
		decision: evaluation.policyEvaluation.decision,
		fail_closed_reason: failClosedReason,
		reason_codes: evaluation.policyEvaluation.reasonCodes,
		evaluated_rules: evaluation.policyEvaluation.evaluatedRules,
	};
}

function buildTransferGatewayVerificationErrorPayload(
	verification: Extract<VerifyTransferGatewayMetadataResult, { ok: false }>,
) {
	return {
		code: verification.reason,
		message:
			verification.reason === "gateway_context_missing"
				? "No existe metadata de gateway/policy para aprobar esta transferencia."
				: "La metadata de gateway/policy no coincide con la propuesta pendiente.",
		suggested_action:
			"Cancelá esta propuesta y generá una transferencia nueva.",
		...(verification.mismatchedFields?.length
			? { mismatched_fields: verification.mismatchedFields }
			: {}),
	};
}

function emitTransferAuditLifecycle(
	input: BuildTransferAuditEventInput,
): TransferAuditEvent | undefined {
	const event = buildTransferAuditEvent(input);
	if (event) {
		recordTransferAuditEvent(event);
	}
	return event;
}

function getTransferAuditEvaluation(
	proposal: PendingProposal,
): TransferGatewayEvaluation | undefined {
	return proposal.proposalType === "transfer"
		? proposal.transferGatewayEvaluation
		: undefined;
}

function emitPendingTransferAuditLifecycle(params: {
	proposal: PendingProposal;
	lifecycle: TransferAuditLifecycle;
	approvalStatus: BuildTransferAuditEventInput["approvalStatus"];
	result: BuildTransferAuditEventInput["result"];
	transactionSignature?: string;
	metadata?: Record<string, unknown>;
}): TransferAuditEvent | undefined {
	const evaluation = getTransferAuditEvaluation(params.proposal);
	if (!evaluation) return undefined;

	return emitTransferAuditLifecycle({
...
async function buildUnsignedSolTransferTx(params: {
	...
	const expiresAtUnix = Math.floor(
		new Date(params.actionMetadata.actionExpiresAt).getTime() / 1000,
	);
	...
	if (params.actionMetadata.includeCreateActionApproval) {
		...
		if (!policyInfo) {
			...
			let policyOffset = 0;
			Buffer.from(initializePolicyDiscriminator).copy(
				initializePolicyData,
				policyOffset,
			);
			policyOffset += 8;
			initializePolicyData.writeBigUInt64LE(
				BigInt(Math.max(lamports, 100 * web3.LAMPORTS_PER_SOL)),
				policyOffset,
			);
			policyOffset += 8;
			initializePolicyData.writeBigUInt64LE(
			...
			policyOffset += 8;
			initializePolicyData.writeUInt16LE(500, policyOffset);
			policyOffset += 2;
			initializePolicyData.writeUInt8(0, policyOffset);
			policyOffset += 1;
			...
			instructions.push(
				new web3.TransactionInstruction({
					...
					keys: [
						...
						{
							pubkey: new web3.PublicKey(policyPda),
							isSigner: false,
							isWritable: true,
						},
						{
	...
	if (params.actionMetadata.includeWalletSafetyAttestation) {
		...
		Buffer.from(attestationDiscriminator).copy(
			attestationData,
			attestationOffset,
		);
		attestationOffset += 8;
		from.toBuffer().copy(attestationData, attestationOffset);
		attestationOffset += 32;
		actionHashBytes.copy(attestationData, attestationOffset);
		attestationOffset += 32;
		to.toBuffer().copy(attestationData, attestationOffset);
		attestationOffset += 32;
		attestationData.writeBigInt64LE(BigInt(expiresAtUnix), attestationOffset);
		attestationOffset += 8;
		attestationData.writeUInt16LE(
			Math.max(
				...
				Math.min(
					10_000,
					Math.round(params.actionMetadata.riskScoreBps ?? 1_500),
				),
			),
			attestationOffset,
		);
		...
		instructions.push(
			new web3.TransactionInstruction({
				...
				keys: [
					...
					{
						pubkey: new web3.PublicKey(attestationPda.address),
						isSigner: false,
						isWritable: true,
					},
					{
						pubkey: getAttestorConfigPda(programId),
						isSigner: false,
						isWritable: false,
					},
					{
	...
	if (params.actionMetadata.includeCreateActionApproval) {
		const createDiscriminator = createHash("sha256")
			...
			.digest()
			.subarray(0, 8);
		const createData = Buffer.alloc(
			8 + 32 + 32 + 1 + 8 + 8 + 2 + 32 + 8 + 32 + 8,
		);
		let offset = 0;
		Buffer.from(createDiscriminator).copy(createData, offset);
		offset += 8;
		new web3.PublicKey(programId).toBuffer().copy(createData, offset);
		offset += 32;
		actionHashBytes.copy(createData, offset);
		offset += 32;
		createData.writeUInt8(5, offset);
		offset += 1;
		createData.writeBigUInt64LE(BigInt(lamports), offset);
		offset += 8;
		createData.writeBigUInt64LE(BigInt(0), offset);
		offset += 8;
		createData.writeUInt16LE(0, offset);
		offset += 2;
		to.toBuffer().copy(createData, offset);
		offset += 32;
		createData.writeBigUInt64LE(BigInt(0), offset);
		offset += 8;
		web3.SystemProgram.programId.toBuffer().copy(createData, offset);
		offset += 32;
		...
		instructions.push(
			new web3.TransactionInstruction({
				...
				keys: [
					...
					{
						pubkey: new web3.PublicKey(actionApprovalPda.address),
						isSigner: false,
						isWritable: true,
					},
					{
...
function parseAttestorKeypair(): web3.Keypair | null {
	const raw =
		process.env.WALLET_SAFETY_ATTESTOR_SECRET_KEY ||
...
export async function ensureWalletSafetyAttestation(params: {
	user: string;
	recipient: string;
	actionHash: string;
	actionExpiresAt: string;
	...
	if (!programId)
		return {
			ok: false,
			reason: "AGENT_ACTION_GUARD_PROGRAM_ID_NOT_CONFIGURED",
		};
	...
	if (!attestor) {
		return {
			ok: true,
			reason: "WALLET_SAFETY_ATTESTOR_SECRET_KEY_NOT_CONFIGURED",
		};
	}
	...
	const expiresAtUnix = Math.floor(
		new Date(params.actionExpiresAt).getTime() / 1000,
	);
	...
	offset += 8;
	new web3.PublicKey(params.user).toBuffer().copy(data, offset);
	offset += 32;
	Buffer.from(params.actionHash, "hex").copy(data, offset);
	offset += 32;
	new web3.PublicKey(params.recipient).toBuffer().copy(data, offset);
	offset += 32;
	data.writeBigInt64LE(BigInt(expiresAtUnix), offset);
	offset += 8;
	data.writeUInt16LE(
		Math.max(0, Math.min(10_000, Math.round(params.riskScoreBps))),
		offset,
	);
		...
		keys: [
			{ pubkey: attestor.publicKey, isSigner: true, isWritable: true },
			{
				pubkey: new web3.PublicKey(
					deriveWalletPolicyPda({ userWallet: params.user }),
				),
				isSigner: false,
				isWritable: false,
			},
...
export async function proxyAgenticChat(
	body: unknown,
	...
	if (!request || typeof request !== "object" || !("type" in request)) {
		return jsonResponse(
			{ error: { code: "invalid_payload", message: "Missing request type" } },
			{ status: 400 },
		);
	}
	...
	const auth: ChatAuthContext = {
		identity: options?.request
			? getAppSessionFromRequest(options.request)
			: null,
	...
	switch (request.type) {
		...
		default:
			return jsonResponse(
				{ error: { code: "invalid_type", message: "Unknown request type" } },
				{ status: 400 },
			);
	}
}
...
async function handleGetHistory(
	...
	const authorizationError = authorizeSessionAccess(
		session,
...
async function handleUserMessage(
	...
	const effectiveUserAddress = getEffectiveWalletAddress(
		auth,
		request.user_address,
	);
	...
	(async () => {
		try {
			...
			let session = sessionId
				? await getSessionWithExternalFallback(sessionId)
				: null;
			...
			if (!session) {
				...
				session = createSession(
					...
					identitySessionMetadata(auth.identity),
				);
			...
			console.log(
				`[chat] User message: ${sessionId} - ${request.content.slice(0, 50)}...`,
			);
			...
			const promptMessages = persistedSession.messages.map(
				(message, index, messages) => {
					const isLatestUserMessage =
						index === messages.length - 1 &&
						message.role === "user" &&
						message.type === "text";
					return isLatestUserMessage
						? { ...message, content: maskedUserInput.content }
						: message;
				},
			);
			...
			const assistantText = await streamResponseToSSE(
				responseStream,
				writer,
				encoder,
			);
...
async function handleOrcaSwapToolCall(
	...
	try {
		...
		const estimatedOutputAmount = uiAmountFromBaseUnits(
			quote.estimated_output_base_units,
			toolArgs.output_token,
		);
		...
		const proposalMessage: AgentFunctionCallMessage = {
			...
			risk: {
				...
				level: "medium",
				reasons: [
					"Ejecución real de swap en Orca devnet",
		...
		const persistedProposalMessage =
			proposalMessageFromFunctionCall(proposalMessage);
		updateSession(sessionId, {
			pendingProposal: {
				...
				toolResult: {
					status: "prepared",
					reason: "READY_FOR_ORCA_SWAP_APPROVAL",
				},
				createdAt: Date.now(),
	...
	try {
		...
		const message =
			error instanceof Error
				? error.message
				: "Unable to fetch wallet holdings.";
...
async function handleGetUsdcSolQuoteToolCall(
	...
	if (
		...
		Number.isNaN(args.input_amount) ||
		args.input_amount <= 0
	) {
		await writeSSE(writer, encoder, "error", {
			code: "invalid_tool_args",
	...
	try {
		const quote = await getUsdcSolQuote({
			...
			...(typeof args.slippage_bps === "number"
		...
		if (
			...
			errorCode === "invalid_quote_payload"
		) {
			await writeSSE(writer, encoder, "error", {
				code: "invalid_tool_args",
				message,
...
async function handleConditionalBuyToolCall(
	...
	if (
		...
		isProposalBlocking(existingPendingProposal) &&
		!isProposalExpired(existingPendingProposal)
	) {
		await writeSSE(writer, encoder, "error", {
			code: "proposal_in_progress",
			message:
				"Ya existe una propuesta pendiente. Resuélvela antes de pedir otra.",
		});
		await writeSSE(writer, encoder, "done", { session_id: sessionId });
		await writer.close();
		return;
	...
	try {
		...
		toolArgs = {
			...
			min_sol_out:
				Number.isFinite(minSolOut) && minSolOut > 0 ? minSolOut : undefined,
			desired_sol_amount:
				Number.isFinite(desiredSolOut) && desiredSolOut > 0
					? desiredSolOut
					: undefined,
			client_order_id:
				Number.isFinite(requestClientOrder) && requestClientOrder > 0
					? Math.floor(requestClientOrder)
					: undefined,
			...
			max_usdc_in:
				Number.isFinite(requestMaxUsdc) && requestMaxUsdc > 0
					? requestMaxUsdc
					: undefined,
			max_oracle_age_seconds:
				Number.isFinite(requestOracleAge) && requestOracleAge > 0
					? requestOracleAge
					: undefined,
			max_confidence_bps:
				Number.isFinite(requestConfidence) && requestConfidence > 0
					? requestConfidence
					: undefined,
	...
	const proposalPayload = toConditionalBuyProposalPayload(
		toolArgs,
		session.userAddress || "",
		quoteUsdPrice,
	);
	...
	if (session.userAddress && requiredUsdc > 0) {
		...
		if (usdcBalance + 0.000001 < requiredUsdc) {
			try {
				...
				const fundingSwap = await buildUsdcFundingSwap(
					missingUsdc,
					toolArgs.target_price_usd,
				);
				...
				const sessionProposalMessage =
					proposalMessageFromFunctionCall(proposal);
				...
				addAgentAlertToSession(sessionId, {
					severity: "info",
					content: `Para crear la orden condicional primero necesitás devUSDC en la wallet. Preparé un swap Orca SOL -> devUSDC; cuando se confirme voy a dejar lista la propuesta condicional.`,
				});
				await persistSessionBestEffort(sessionId);
...
async function handleTransferToolCall(
	...
	if (toolResult.status !== "prepared") {
		...
		const currentConversationInput = toHistoryPrompt(
	...
	try {
		...
		await writeSSE(writer, encoder, "error", {
			...
			message:
				error instanceof Error
					? error.message
					: "No se pudo configurar el guardrail on-chain.",
		});
		await writeSSE(writer, encoder, "done", { session_id: sessionId });
		await writer.close();
		return;
	...
	if (normalizeTransferToken(toolArgs.token).toUpperCase() === "SOL") {
		...
		try {
			...
			await writeSSE(writer, encoder, "error", {
				...
				message:
					"No pude verificar el saldo de tu wallet en devnet. Reintentá en unos segundos antes de preparar la transferencia.",
			});
			await writeSSE(writer, encoder, "done", { session_id: sessionId });
			await writer.close();
			return;
	...
	if (safety.decisionResult.decision === "REJECT") {
		const riskReasons = safety.decisionResult.reasons.map(
	...
	try {
		actionMetadata = buildTransferMetadata(canonical, proposalCreatedAt, {
			policyPda,
		});
	} catch (error) {
		await writeSSE(writer, encoder, "error", {
	...
	const transferGatewayEvaluation = await evaluateTransferGateway({
		id: `transfer-${actionHash}`,
		network: DEFAULT_SOLANA_NETWORK,
		toolName: toolCall.name,
		actorWallet: userAddress,
		amountSol: canonical.amount,
		recipientAddress: canonical.recipient,
		recipientKnown: deriveTransferRecipientKnown(safety),
		createdAt: actionMetadata.actionCreatedAt,
		walletSafety: buildTransferWalletSafetyEvidence(safety),
		quoteUsd: async () => {
			const quote = await getUsdcSolQuote({
				network: DEFAULT_SOLANA_NETWORK,
				input_token: "SOL",
				output_token: "USDC",
				input_amount: canonical.amount,
			});
	...
	if (!transferGatewayEvaluation.proposalEligible) {
		emitTransferAuditLifecycle({
			lifecycle: TRANSFER_AUDIT_LIFECYCLES.PROPOSAL_REJECTED,
			evaluation: transferGatewayEvaluation,
			approvalStatus: "not_required",
			result: "denied",
			metadata: {
				failClosedReason:
					transferGatewayEvaluation.failClosedReason ??
					"policy_unhandled_decision",
				policyReasonCodes:
					transferGatewayEvaluation.policyEvaluation.reasonCodes,
				evaluatedRules:
					transferGatewayEvaluation.policyEvaluation.evaluatedRules,
			},
		});
		await writeSSE(
			writer,
			encoder,
			"error",
			buildTransferPolicyErrorPayload(transferGatewayEvaluation),
		);
		await writeSSE(writer, encoder, "done", { session_id: sessionId });
		await writer.close();
		return;
	...
	const explanation = await attachGuardrailNarration(
		buildTransferGuardrailExplanation({
			risk,
	...
	const pendingProposal: PendingProposal = {
		...
		actionCreatedAt: actionMetadata.actionCreatedAt,
		actionExpiresAt: actionMetadata.actionExpiresAt,
		transferGatewayMetadata: transferGatewayEvaluation.metadata,
	...
	const proposal: AgentFunctionCallMessage = {
		...
		risk: {
			...
			requiresExtraConfirmation:
				safety.decisionResult.requiresExtraConfirmation,
			explanation,
		},
		execution: {
			mode: "phantom_sign_and_send",
	...
	await persistSessionBestEffort(sessionId);
	emitTransferAuditLifecycle({
		lifecycle: TRANSFER_AUDIT_LIFECYCLES.PROPOSAL_CREATED,
		evaluation: transferGatewayEvaluation,
	...
	console.log(
		`[chat] Proposal created: ${sessionId} - transfer ${canonical.amount} ${canonical.token}`,
	);
...
async function handleFunctionApprove(
	request: {
		type: "function_approve";
		session_id: string;
		action_hash?: string;
	...
	if (!request.session_id?.trim()) {
		return jsonResponse(
			{ error: { code: "invalid_payload", message: "session_id is required" } },
			{ status: 400 },
		);
	}
		...
		return jsonResponse(
			{
				error: {
					...
					message: "No pending proposal for this session",
				},
			},
			{ status: 400 },
		);
		...
		return jsonResponse(
			{
				error: {
					...
					message: "Pending proposal expired",
				},
			},
			{ status: 400 },
		);
	...
	if (pendingProposal.state === "guard_rejected_awaiting_bypass") {
		if (!request.accept_risk) {
			console.log(
				`[swapGuard BYPASS] User declined bypass for session ${request.session_id}`,
			);
			clearPendingProposal(request.session_id);
			return jsonResponse(
				{
					messages: [
						{
							type: "text",
							content:
								"Swap cancelado. El guard de precio protegió tu transacción.",
							timestamp: now(),
						},
					],
		...
		console.log(
			`[swapGuard BYPASS] User ACCEPTED risk for session ${request.session_id}`,
		);
		console.log(
			`[swapGuard BYPASS] guardRejection=${JSON.stringify(pendingProposal.guardRejection)}`,
		);
			...
			return jsonResponse(
				{
					error: {
						code: "no_wallet",
						message: "No wallet connected in session",
					},
				},
				{ status: 400 },
			);
		...
		try {
			...
			console.log(
				`[swapGuard BYPASS] Building UNGUARDED swap tx: ${swapArgs.input_amount} ${swapArgs.input_token} -> ${swapArgs.output_token}`,
			);
			...
			return jsonResponse(
				{
					...
					swap_execution: {
						...
						quote:
							(swapArgs as OrcaSwapParams & { quote?: OrcaSwapQuote }).quote ??
							null,
					},
					transaction: {
						format: unsigned.isVersioned
							? "base64_versioned_transaction"
							: "base64_legacy_transaction",
			...
			return jsonResponse(
				{
					error: {
						...
						message:
							e instanceof Error
								? e.message
								: "Failed to build unguarded swap transaction",
					},
				},
				{ status: 500 },
			);
	...
	if (actionExpiry && isPendingActionExpired(actionExpiry)) {
		...
		return jsonResponse(
			{
				error: {
					code: "pending_proposal_expired",
					message: "La propuesta expiró y ya no puede aprobarse.",
				},
			},
			{ status: 400 },
		);
	...
	if (
		pendingProposal.state !== "awaiting_approval" &&
		pendingProposal.state !== "failed"
	) {
		return jsonResponse(
			{
				error: {
					code: "proposal_state_conflict",
					message: `Cannot approve proposal in state ${pendingProposal.state}`,
				},
			},
			{ status: 400 },
		);
	...
	const proposal = (await getSessionWithExternalFallback(request.session_id))
		?.pendingProposal;
	if (!proposal) {
		return jsonResponse(
			{
				error: {
					code: "session_not_found",
					message: "Session not found while preparing proposal",
				},
			},
			{ status: 404 },
		);
	...
	if (toolName === "conditional_buy_sol") {
		...
		if (
			proposal.expectedUserAddress &&
			proposal.expectedUserAddress !== session.userAddress
		) {
			updateSession(request.session_id, {
				pendingProposal: {
					...proposal,
			...
			return jsonResponse(
				{
					error: {
						code: "wallet_mismatch",
						message:
							"Connected wallet does not match expected wallet for this proposal.",
					},
				},
				{ status: 400 },
			);
		...
		try {
			...
			const expiresAtUnix =
				typeof buyArgs.expires_at === "string"
			...
			return jsonResponse(
				{
					error: {
						...
						message:
							e instanceof Error
								? e.message
								: "Failed to build conditional order transaction",
					},
				},
				{ status: 500 },
			);
		...
		const response: {
			...
			transaction?: {
				...
				network: "devnet" | "mainnet-beta";
				onchain_guardrail?: NonNullable<
					AgentFunctionCallMessage["onchain_guardrail"]
				>;
			};
		} = {
			messages: [
	...
	if (toolName === "swap_orca_usdc_to_sol") {
		...
		const maxDeviationBps = Number(
			process.env.SWAP_GUARD_MAX_DEVIATION_BPS || "500",
		);
		const warningDeviationBps = Number(
			process.env.SWAP_GUARD_WARNING_DEVIATION_BPS || "150",
		);
		...
		const stalenessSeconds = Number(
			process.env.SWAP_GUARD_STALENESS_SECONDS || "9999999999",
		);
		const maxConfidenceBps = Number(
			process.env.SWAP_GUARD_MAX_CONFIDENCE_BPS || "10000",
		);
		...
		try {
			if (swapGuardEnabled) {
				...
				const inputAmountBaseUnits = BigInt(
					Math.round(
						swapArgs.input_amount * (inputIsUsdc ? 1_000_000 : 1_000_000_000),
					),
				);
				...
				const quotedPriceUsdE8 = BigInt(
					Math.round(impliedPriceUsd * 100_000_000),
				);

				console.log(
					`[swapGuard ON-CHAIN] Building guarded swap tx: ${swapArgs.input_amount} ${swapArgs.input_token} -> ${swapArgs.output_token}`,
				);
				console.log(
					`[swapGuard ON-CHAIN] Implied price: $${impliedPriceUsd.toFixed(4)} (${quotedPriceUsdE8} e8)`,
				);
				console.log(
					`[swapGuard ON-CHAIN] Max deviation: ${maxDeviationBps} bps, Staleness: ${stalenessSeconds}s`,
				);
				...
				if (guardInstructions.initializePolicyIx) {
					console.log(
						"[swapGuard ON-CHAIN] Including initialize_policy instruction (first-time user)",
					);
				...
				console.log(
					`[swapGuard ON-CHAIN] Total guard instructions: ${ixList.length}`,
				);

				// Get oracle price for comparison (for error reporting if guard rejects)
				let oraclePriceUsd = impliedPriceUsd; // fallback
				try {
					const serverGuardResult = await buildSwapGuardConfig(
						impliedPriceUsd,
						swapArgs.input_token,
						swapArgs.output_token,
					);
					if (serverGuardResult.config.oracle_price_usd_e8) {
						oraclePriceUsd =
							serverGuardResult.config.oracle_price_usd_e8 / 100_000_000;
					}
				} catch (e) {
					console.warn(
						"[swapGuard ON-CHAIN] Could not fetch oracle price for comparison:",
						e,
					);
				}
				...
				if (!txResult.success) {
					if ("guardRejection" in txResult && txResult.guardRejection) {
						console.log(
							`[swapGuard ON-CHAIN] Guard REJECTED transaction - deviation too high`,
						);
						console.log(
							`[swapGuard ON-CHAIN] deviationBps=${txResult.guardRejection.deviationBps}, maxAllowed=${txResult.guardRejection.maxAllowedBps}`,
						);
						console.log(
							`[swapGuard ON-CHAIN] quotedPrice=$${txResult.quotedPriceUsd.toFixed(2)}, oraclePrice=$${txResult.oraclePriceUsd.toFixed(2)}`,
						);
						...
						const deviationPercent = (
							txResult.guardRejection.deviationBps / 100
						).toFixed(2);
						...
						const guardRejectionWithExplanation = {
							...guardRejection,
							explanation: await attachGuardrailNarration(
								buildSwapGuardRejectionExplanation({
					...
					const errorMsg =
						"error" in txResult
							? txResult.error
							: "Failed to build Orca swap transaction with guard";
				...
				try {
					...
					if (serverGuardResult.warning) {
						...
						console.log(
							`[swapGuard ON-CHAIN] Pre-check warning: ${swapGuardWarning.message}`,
						);
					}
					// Update config with oracle price from server check
					if (serverGuardResult.config.oracle_price_usd_e8) {
						swapGuardConfig.oracle_price_usd_e8 =
							serverGuardResult.config.oracle_price_usd_e8;
						swapGuardConfig.deviation_bps =
							serverGuardResult.config.deviation_bps;
					}
				} catch (e) {
					console.warn(
						"[swapGuard ON-CHAIN] Server-side pre-check failed (non-blocking):",
						e,
					);
				}

				console.log(
					"[swapGuard ON-CHAIN] Transaction built successfully with on-chain guard",
				);
			} else {
			...
			console.error(
				"[swapGuard ON-CHAIN] Error building guarded transaction:",
				e,
			);
			updateSession(request.session_id, {
				pendingProposal: {
		...
		const response: {
			...
			swap_guard_warning?: NonNullable<SwapGuardWarning> & {
				explanation?: GuardrailExplanation;
			};
		} = {
			messages: [
	...
	const transferSafetyDecision = (
		proposal.toolResult as
			| { walletSafety?: WalletSafetyDecisionResult }
			| undefined
	)?.walletSafety;
	if (transferSafetyDecision?.decision === "REJECT") {
		...
		return jsonResponse(
			{
				error: {
					code: "proposal_rejected_by_policy",
					message: "La propuesta no puede aprobarse por policy de seguridad.",
				},
			},
			{ status: 400 },
		);
	...
	const expectedActionHash = buildTransferActionHash(
		canonicalForApprove,
		createdAt,
		{ policyPda },
	);
	if (
		!pendingActionHash ||
		hasActionHashMismatch(expectedActionHash, pendingActionHash)
	) {
		clearPendingProposal(request.session_id);
		return jsonResponse(
			{
				error: {
					...
					message: "La propuesta fue alterada.",
				},
			},
			{ status: 409 },
		);
	...
	if (
		request.action_hash &&
		hasActionHashMismatch(request.action_hash, pendingActionHash)
	) {
		clearPendingProposal(request.session_id);
		return jsonResponse(
			{
				error: {
					code: "action_hash_mismatch",
					message: "El hash de aprobación no coincide con la propuesta.",
				},
			},
			{ status: 409 },
		);
	...
	const currentTransferGatewayMetadata = transferGatewayMetadata
		? buildTransferGatewayApprovalMetadata({
				stored: transferGatewayMetadata,
				candidateId: `transfer-${pendingActionHash}`,
				network: proposal.network,
				toolName,
				actorWallet: session.userAddress,
				amountSol: transferArgs.amount,
				recipientAddress: transferArgs.recipient,
				createdAt: actionCreatedAt || new Date(createdAt).toISOString(),
			})
		: undefined;
	const transferGatewayVerification = verifyTransferGatewayMetadata({
		stored: transferGatewayMetadata,
		current: currentTransferGatewayMetadata,
	});
	if (transferGatewayVerification.ok === false) {
		updateSession(request.session_id, {
			pendingProposal: {
				...proposal,
				state: "failed",
		...
		return jsonResponse(
			{
				error: buildTransferGatewayVerificationErrorPayload(
					transferGatewayVerification,
				),
			},
			{ status: 409 },
		);
	...
	emitPendingTransferAuditLifecycle({
		proposal,
		lifecycle: TRANSFER_AUDIT_LIFECYCLES.APPROVAL_RECEIVED,
		...
		return jsonResponse(
			{
				error: {
					...
					message:
						"No existe contexto de guardrail on-chain para esta propuesta.",
				},
			},
			{ status: 400 },
		);
	...
	const expectedAmountLamports = Math.round(
		transferArgs.amount * web3.LAMPORTS_PER_SOL,
	);
	const resolvedActionApprovalPda =
		actionApprovalPda ||
		deriveActionApprovalAddress({
			user: session.userAddress,
			actionHash: pendingActionHash,
		}).address;
	const resolvedWalletSafetyAttestationPda =
		walletSafetyAttestationPda ||
		deriveWalletSafetyAttestationAddress({
			user: session.userAddress,
			recipient: transferArgs.recipient,
	...
	try {
		unsignedTx = await buildUnsignedSolTransferTx({
			...
			actionMetadata: {
				...
				includeWalletSafetyAttestation: Boolean(
					readiness.walletSafetyAttestationMissing,
				),
				riskScoreBps:
					transferSafetyDecision?.riskLevel === "critical"
						? 8_500
						: transferSafetyDecision?.riskLevel === "medium"
							? 5_000
							: 1_500,
			},
		});
	...
	emitPendingTransferAuditLifecycle({
		proposal,
		lifecycle: TRANSFER_AUDIT_LIFECYCLES.UNSIGNED_TX_PREPARED,
		approvalStatus: "approved",
		result: "pending",
		metadata: {
...
async function handleFunctionResult(
	...
	const pendingProposal = session.pendingProposal;
	if (
		pendingProposal.toolName === "conditional_buy_sol" &&
		...
		addSessionMessagesFromAgentMessages(
			request.session_id,
			declineResponse.messages,
		);
		await persistSessionBestEffort(request.session_id);
		return jsonResponse(
			{
	...
	if (
		...
		request.status === "confirmed" &&
		pendingProposal.toolResult?.reason ===
			"DEV_USDC_REQUIRED_BEFORE_CONDITIONAL_ORDER"
	) {
		const nextArgs = pendingProposal.toolResult.next_conditional_buy_args as
			| ConditionalBuySolParams
			| undefined;
		if (nextArgs) {
				...
				messages: [
					{
						type: "text",
						content:
							"Swap de devUSDC confirmado. Ahora queda lista la orden condicional para aprobar con Phantom.",
						execute: { status: "success", tx_hash: request.tx_signature },
						timestamp: now(),
					},
					proposal,
	...
	if (pendingProposal.proposalType === "transfer") {
		const lifecycle =
			request.status === "confirmed"
				? TRANSFER_AUDIT_LIFECYCLES.RESULT_CONFIRMED
				: request.status === "failed"
					? TRANSFER_AUDIT_LIFECYCLES.RESULT_FAILED
					: TRANSFER_AUDIT_LIFECYCLES.RESULT_SUBMITTED;
		emitPendingTransferAuditLifecycle({
			proposal: pendingProposal,
			lifecycle,
			approvalStatus: "approved",
			result: request.status,
			transactionSignature: request.tx_signature,
			metadata: {
				status: request.status,
				hasErrorMessage: Boolean(request.error_message),
			},
		});
	}
		...
		messages: [
			{
				...
				execute: {
					status:
						request.status === "confirmed" || request.status === "submitted"
							? "success"
							: "failed",
...
async function handleFunctionReject(
	request: {
		type: "function_reject";
		session_id: string;
		reason?: string;
	...
	emitPendingTransferAuditLifecycle({
		proposal: pendingProposal,
		lifecycle: TRANSFER_AUDIT_LIFECYCLES.USER_REJECTED,
	...
	console.log(
		`[chat] Proposal rejected: ${request.session_id}${request.reason ? ` - ${request.reason}` : ""}`,
	);
		...
		messages: [
			{
				type: "text",
				content:
					"Entendido, cancelé la transferencia. ¿Hay algo más en lo que pueda ayudarte?",
				timestamp: now(),
			},
		],
	};
