...
| T2 | [`T2-phantom-auth-balances.md`](./task-specs/T2-phantom-auth-balances.md) | Phantom Embedded auth/display + balances desde backend. |
| T3 | [`T3-chat-mocked-backend.md`](./task-specs/T3-chat-mocked-backend.md) | Chat store + protocolo `/api/agent/message` mockeado. |
...
- El frontend es UI/chat + Phantom Embedded auth/display.
...
- El frontend consume endpoints propios `/api/*`.
- Backend/agent decide risk, quotes, auto-execute vs confirmación y ejecución real.
